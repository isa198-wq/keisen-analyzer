...
実装時に決めるべきこと:
- **日次日記ページの特定方法**。現状タイトル命名が不統一(`20260710`=YYYYMMDD / `0708`=MMDD / `6/5`=M/D が混在)。GASで「今日のページを探す→無ければ作る」を安定させるには、命名を1つに統一するか、探索ルール(親ページ配下をタイトル前方一致で検索等)を確定する必要がある。日記ページ群の親(「記録」ページ配下か等)も要確認。
