...
// --- LINE通知本文のセクション別cap（4900字上限に収めるための行数制限） ---
// 実測: 買い/売り行≈40字、三尊/逆三尊行≈62字。最重量日(2026-07-03: 買50売30三尊24逆22)は
// 旧cap(買売20・パターン無制限相当の50)で5,610字と超過した。固定部(見出し・🆕・解除・張り付き)
// ≈1,300字＋パターン15×2×63字＋買売15×2×40字≈4,500字で、重い日でも4900字に収まる。
...
async function notify() {
  ...
  const patLines = (rows, breakWord, arrow) => {
    const cap = LINE_CAP_PAT;  // 4900字対策のセクション別cap（冒頭の定数を参照）
    const lines = rows.slice(0, cap).map((r) => {
